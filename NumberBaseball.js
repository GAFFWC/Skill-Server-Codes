...
// 각 자릿수에 0이 없고, 서로 중복되지 않을 때까지 4자리 수를 뽑아서 반환
const GetNewChoice = () => {
    let temp = Math.floor(Math.random() * 10000);
    while (!CheckDuplicated(String(temp)) || temp < 1000) {
...
router.post("/game", (req, res) => {
    ...
    // 길이가 4보다 길다
    if (!ErrorText && userText.length > 4) ErrorText = "숫자의 길이가 너무 길어요.\n4자리 숫자를 입력해주세요.";

    // 길이가 4보다 짧다
    if (!ErrorText && userText.length < 4) ErrorText = "숫자의 길이가 너무 짧아요.\n4자리 숫자를 입력해주세요.";

    // 각 자릿수 중 중복되는 숫자가 있음
    if (!ErrorText && !CheckDuplicated(userText)) ErrorText = "숫자의 각 자릿수 중 중복되는 숫자가 있어요.\n각 자릿수가 중복되지 않도록 다시 입력해주세요.";
...
router.post("/record", (req, res) => {
    ...
    for (const i of record) description += i + "\n";
...
router.post("/start", (req, res) => {
    ...
    InitGame(userId);
    res.json(GameMessage("게임을 시작합니다.\n4자리 숫자를 입력해주세요."));
});

router.post("/continue", (req, res) => {
    // 이어 하기 (설명 보던 중 이어할 수 있도록)
    const userId = req.body.userRequest.user.id;
...
const PlayGame = (userText, userId) => {
    // 게임 결과, 목숨 안내
    let score = CalculateScore(userText, String(botChoice[userId]));
    let description = "";
